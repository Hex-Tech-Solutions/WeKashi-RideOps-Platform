 ...
 *   Escort required in window when: no male exists (all-female or lone female).
 *
...
export function evaluateEscortPolicy(
  ...
  if (!inRestrictedWindow(rideTime)) {
    // Still reorder the route for safety (male first on login, females first on logout)
    return { required: false, reordered: maleCount > 0 };
  }

  // ── Inside restricted window (19:00–07:00) ────────────────────────────────

  // No male exists → cannot buffer → escort required
  if (maleCount === 0) {
  ...
  // ≥1 male exists → reorder handles it (male at seq=0 for login, males last for logout)
